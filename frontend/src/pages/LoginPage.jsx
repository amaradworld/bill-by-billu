...
export default function LoginPage() {
  ...
  const handleGoogleLogin = useCallback(async () => {
    ...
    try {
      ...
      console.error('Google login error:', msg, code);
      if (code === '12501' || /cancel/i.test(msg)) {
        setGoogleLoading(false);
        return;
      }
      if (code === 10 || /developer_error/i.test(msg) || /10/i.test(msg)) {
        toast.error('Native sign-in unavailable. Switching to web sign-in...');
        setUseNativeAuth(false);
        setGoogleLoading(false);
        return;
      }
      toast.error(`Google login failed: ${msg || 'Unknown error'}`);
      setGoogleLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    if (useNativeAuth || googleInitRef.current) return;
          ...
          <div className="mb-5">
            {useNativeAuth ? (
              <button onClick={handleGoogleLogin} disabled={googleLoading}
                className="btn-press w-full flex items-center justify-center gap-3 px-4 py-3 border border-gray-200 rounded-xl hover:bg-gray-50 hover:border-gray-300 transition-all duration-200 disabled:opacity-50">
