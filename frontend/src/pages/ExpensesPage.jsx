...
export default function ExpensesPage() {
  ...
  const handleCreate = (e) => {
    ...
    const gstAmt = amt * Number(form.gstRate) / 100;
    setExpenses(prev => [...prev, { ...form, id: Date.now().toString(), gstAmount: gstAmt, totalAmount: amt + gstAmt }]);
    setShowForm(false);
  ...
  const handleDelete = (id) => {
    if (confirm(t('common.confirm'))) setExpenses(prev => prev.filter(e => e.id !== id));
  };
            ...
            <tbody className="divide-y">
              {expenses.map(e => (
                <tr key={e.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-right text-gray-500">{fmt(e.gstAmount)}</td>
                  <td className="px-4 py-3 text-center"><button onClick={() => handleDelete(e.id)} className="text-red-400 hover:text-red-600 text-xs">{t('common.delete')}</button></td>
                </tr>
              ))}
            </tbody>
          </table>
